...
void Producer::ProduceCluster(int MPA_no, int hits_per_event, UShort_t BX_ID)
{
...
//                 Calculate pixel coordinate and weight
                ...
                clustervec.at(index).size_x+=tmp.area;
                clustervec.at(index).size_y+=tmp.area;
                if(hits_per_event==1)
                    break;
            }
        }
    ...
    {
        cluster.cog_x=cluster.cog_x/cluster.size_x;
        cluster.cog_y=cluster.cog_y/cluster.size_y;
...
void Producer::FillMemoryHists(const MemoryNoProcessingBranch_t& MemoryNoProcessingBranch, int MPA_no)
{
    ...
    for(auto i=0; i<MEMORY; i++){
        ...
        if(hits_per_event>3){
            ProduceCluster(MPA_no,hits_per_event, MemoryNoProcessingBranch.bunchCrossingId[i]);
          }
      }
}
