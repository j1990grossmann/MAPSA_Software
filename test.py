...
def read_file(arr,meas_type):
    ...
    #channelcounts_norm = TH2F('HitMap_norm','Counts per 25 ns; Channel; DAC Value (a.u.)', 288, .5,288.5,256, .5, 256.5)
    ...
    stack = THStack('a',';DAC Value (a.u.);Counts per 25 ns ')
    fitfuncs = []
    fitparams = []
    gr1 = []
    gr2 = []
    for pixel in range(0, channels):
        ...
        gr2.append(TH1F('th1f_'+str(pixel).zfill(3),str(pixel+1).zfill(3)+';DAC Value (a.u.);Counts per 25 ns',256,0.5,256.5))
        color=pixel%8+1
